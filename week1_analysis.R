...
qqPlot(d$FreeSpirit, distribution = "norm",
...
qqPlot(d$TransformOfBorendom, distribution = "unif",

       main = "Uniform Q-Q Plot of TB")

print(uniform)

d$Achiever_normalized <- (d$Achiever - min(d$Achiever)) / (max(d$Achiever) - min(d$Achiever))

beta1<-fitdist(d$Achiever_normalized, "beta")

qqPlot(d$Achiever_normalized, distribution = "beta", 
...
qqPlot(d$TransformOfChallenge_normalized, distribution = "beta", 
...
ks_fs <- ks.test(d$FreeSpirit, "pnorm", mean = normal$estimate["mean"], sd = normal$estimate["sd"])

cat("\nKolmogorov-Smirnov test for FreeSpirit:\n")

print(ks_fs)

ks_tb <- ks.test(d$TransformOfBorendom, "punif", min = uniform$estimate["min"], max = uniform$estimate["max"])

cat("Kolmogorov-Smirnov test for TransformOfBorendom (uniform):\n")

print(ks_tb)

ks_ac <- ks.test(d$Achiever_normalized, "pbeta", 
...
ks_tc <- ks.test(d$TransformOfChallenge_normalized, "pbeta",
